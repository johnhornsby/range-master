...
export default class RangeMaster extends EventEmitter {
 ...
 _______________________________________________*/
	...
	_init(options) {
		...
		const rangeDogOptions = _extends({}, this._options);
	...
	_onDragMove(deltaX, input) {
		...
		const deltaData = { x: deltaX, time: new Date().getTime(), input: input };
	...
	_activatePostDragBehaviour() {
		...
		if (this._snap) {
			const inertiaDirection = deltaInfo.x;
